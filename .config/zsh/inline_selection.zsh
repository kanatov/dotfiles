...
() {
  ...
  .zshrc_def-deselect-and cmd-left-word  .backward-word '^[[1;5D'
  .zshrc_def-deselect-and cmd-right-word .forward-word  '^[[1;5C'

  # -- Cmd+Shift+left/right = Select word
  .zshrc_def-select-and cmd-left-word    .backward-word '^[[1;6D'
  .zshrc_def-select-and cmd-right-word   .forward-word  '^[[1;6C'

  # -- Cmd+A = Select all (Kitty sends \x01)
  bindkey '\x01' .zle_select-all

}

zle-copy-region-to-clipboard() {
  if (( REGION_ACTIVE )); then
    ...
    local selection="${BUFFER[start+1,end]}"
    print -rn -- "$selection" | pbcopy
    (( REGION_ACTIVE=0 ))
  fi
}
zle -N zle-copy-region-to-clipboard
bindkey '\e[99~' zle-copy-region-to-clipboard

zle-cut-region-to-clipboard() {
  if (( REGION_ACTIVE )); then
    local start=$CURSOR
    local end=$MARK
    if (( start > end )); then
...
zle -N zle-cut-region-to-clipboard
bindkey '\e[98~' zle-cut-region-to-clipboard  # ESC[98~ will be sent by Cmd+X
